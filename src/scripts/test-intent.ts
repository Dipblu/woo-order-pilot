...
const TEST_CASES: TestCase[] = [
  ...
  {
    name: 'quantity after "order" is not an order number (fixed)',
    message: 'can I order 20 pieces of lumpia?',
    pending: { pending_intent: 'order_status' },
    expect: { intent: 'faq', isContinuation: false, orderNumber: null },
  },
  { name: 'quantity: want to order N', message: 'I want to order 20 lumpia', pending: { pending_intent: 'order_status' }, expect: { orderNumber: null } },
  { name: 'quantity: like to order N pcs', message: 'id like to order 50 pcs', pending: { pending_intent: 'order_status' }, expect: { orderNumber: null } },
  { name: 'quantity: please order N servings', message: 'please order 12 servings', pending: { pending_intent: 'order_status' }, expect: { orderNumber: null } },
  { name: 'quantity: unit follows with no verb cue', message: 'order 20 pieces', pending: { pending_intent: 'order_status' }, expect: { orderNumber: null } },
  { name: 'quantity: would like to order N boxes', message: 'we would like to order 30 boxes', pending: { pending_intent: 'order_status' }, expect: { orderNumber: null } },
